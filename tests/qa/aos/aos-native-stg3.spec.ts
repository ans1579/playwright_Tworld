...
test(`Native AOS 064: 정회원 로그인 후 TWD/TDS 각 메인에서 바텀 navi AI 추천 버튼 선택`, async ({ driver }) => {
    ...
    expect(await isVisible(driver, `//android.widget.Spinner[@text="전체 AI 추천"]`, 15000)).toBe(true);
    ...
    await safeClick(driver, aiBtn);
    expect(await isVisible(driver, `//android.widget.Spinner[@text="T 다이렉트샵"]`, 15000)).toBe(true);
})
