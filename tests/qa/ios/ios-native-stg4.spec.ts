...
test(`Native iOS 064: 비로그인 상태에서 넛징 선택`, async ({ driver }) => {
    ...
    const resetQueued = await driver.execute(() => {
        ...
        const btn = [...document.querySelectorAll("button")].find((el) => (el.textContent || "").trim() === "nudge 초기화") as HTMLElement | undefined;
...
test(`Native iOS 068: 준회원 로그인 후 메시지 넛징 선택`, async ({ driver }) => {
    await safeClick(driver, `//XCUIElementTypeOther[@name="메뉴 탭"]`);
    if (!(await isVisible(driver, logout))) {
        ...
        await safeClick(driver, `//XCUIElementTypeOther[@name="홈 탭"]`);
    } else if (!(await isVisible(driver, `(//XCUIElementTypeOther[@name="문지훈 님"])[2]`))) {
        await safeClick(driver, logout);
        await safeClick(driver, `//XCUIElementTypeStaticText[@name="예(Y)"]`);
        await safeClick(driver, `//XCUIElementTypeStaticText[@name="홈으로"]`);
    ...
    await driver.activateApp(TWD);
    await safeClick(driver, nudgeMessage);
    expect(await isVisible(driver, `//XCUIElementTypeButton[@name="회선 등록하기"]`)).toBe(true);
});
...
test(`Native iOS 071: 같은 계정으로 앱 재실행`, async ({ driver }) => {
    ...
    const visible = await driver
        ...
        .isDisplayed()
        .catch(() => false);
    const ctx71 = await driver.getContext().catch(() => "확인불가");
    console.log(`[Test:071] 넛징 노출여부: ${visible}, 컨텍스트: ${ctx71}`);
    expect(visible).toBe(false);
});

test(`Native iOS 072: TWD / TDS 각 채널에 맞는 넛징 메세지 노출 확인`, async ({ driver }) => {
    const twdMsg = ['"AI 추천이 발견했어요! 내게 필요한 것 모아 보기"의 썸네일 이미지', '"T 월드·T 다이렉트샵·T 멤버십에서 AI가 필요한 정보만 모았어요"의 썸네일 이미지', '"혹시 변경해야 할 정보는 없는지 AI T 서비스로 체크해 볼까요?"의 썸네일 이미지', '"고객센터에 문의했던 내용 AI T 서비스가 해결해 드려요"의 썸네일 이미지', '"구매부터 요금제 관리, 혜택까지 AI가 나의 T 서비스를 밀착 관리"의 썸네일 이미지', '"T 서비스 1000% 활용 방법! 나에게 맞는 정보만 모아 드려요"의 썸네일 이미지', '"시간 절약을 위한 맞춤 솔루션 AI가 필요한 기능을 찾아왔어요"의 썸네일 이미지', '"꼭 필요한 순간에 맞춤 제안 시간·상황에 맞춰 알려 드려요"의 썸네일 이미지', '"T의 멈추지 않는 추천 매일이 새로워지는 탐색 시작!"의 썸네일 이미지', '"워치·태블릿, 제대로 활용하려면? 전용 요금제를 추천드려요"의 썸네일 이미지'];
    const tdsMsg = ['"헌 집 줄게 새집 다오! 쓰던 폰 보상받고 새 폰 주문하기!"의 썸네일 이미지'];
