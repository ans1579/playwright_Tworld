...
test.describe(`Native iOS 080`, () => {
  ...
  test(`Native iOS 080: iOS 알림 허용 끄고 AI Layer 진입`, async ({ driver }) => {
    ...
    const switchValue = String(
      (await notificationSwitch.getAttribute("value")) ?? "",
    )
      .trim()
      .toLowerCase();
    if (switchValue === "1") {
    ...
    await driver.pause(5000);
    if (
      await isVisible(driver, `//XCUIElementTypeStaticText[@name="시작하기"]`)
    ) {
      await safeClick(driver, `//XCUIElementTypeStaticText[@name="시작하기"]`);
    }
    if (
      await isVisible(driver, `//XCUIElementTypeStaticText[@name="‘[STG] T world’ 앱이 사용자의 위치를 사용하도록 허용하겠습니까?"]`)
    ) {
      await driver
    ...
    if (
      await isVisible(driver, `//XCUIElementTypeStaticText[@name="AI가 제안"]`)
    ) {
